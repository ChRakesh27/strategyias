...
async function getTopper({ query, page, limit }) {
  ...
  const pipeline = [{ $skip: skip }, { $limit: limit },{ $sort: { gs3marks: -1 } }];
  if (query) {
    pipeline.unshift({
      $search: {
        index: "searchTopper",
      ...
      <div className={styles.Wrapper}>
        <div className={styles.PageHeadingContainer}>
          <div className={styles.PageHeading}>
            <h1>
              UPSC 2022 TOPPERS <span>GS-3 WISE</span>
            </h1>
          </div>
        </div>
        ...
        <div className={styles.paginationDiv}>
          {page > 1 && (
            <Link href={`/toppers-gs3wise?page=${page - 1}`}>
          ...
          {pageNumbers.map((pageNumber) => {
            ...
            return isPageValid ? (
              <Link key={pageNumber} href={`/toppers-gs3wise?page=${pageNumber}`}>
                <div
